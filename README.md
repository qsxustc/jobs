# 秋招助手（macOS）

一个使用 SwiftUI 编写的原生 macOS 秋招求职记录工具。数据只保存在本机，不依赖账号或服务器。当前版本为 2.0。

## 已实现功能

- 公司、招聘项目和岗位投递记录
- 分别记录岗位薪资范围与实际 Offer 薪资
- 状态筛选、搜索和可拖拽流程看板
- 多轮面试、笔试、测评与 Offer 沟通记录
- 待办事项、截止时间和 macOS 本地通知
- 首页仪表盘、过久未更新提示和近期日程
- 投递趋势、回复率、面试率、Offer 率和渠道分析
- CSV 导入导出与完整 JSON 备份恢复
- 快速新建投递，粘贴 JD 自动识别公司、岗位、地点、类别、招聘项目、截止时间和岗位要求
- 新建投递查重，可合并到已有记录或继续创建
- 滚动本地自动备份、备份时间展示和历史版本恢复
- 自动保存和状态变更历史
- 自定义流程状态和看板列
- 岗位多标签及筛选
- 简历版本管理与投递关联
- 月历视图
- 应用内智能提醒中心
- 技术面、产品面和 HR 面复盘模板
- 从剪贴板离线识别面试、笔试和测评通知，可校对后写入投递流程
- 按岗位类别、城市和简历版本分析
- 第一版数据自动迁移
- 后台节能：窗口关闭后可按设置自动退出，窗口保留时使用 macOS App Nap

完整开发进度见 [`docs/TODO.md`](docs/TODO.md)。

## 构建

要求 macOS 14 或更高版本，以及 Xcode 16 或更高版本。

```bash
./Scripts/build_app.sh
```

构建结果位于 `dist/秋招助手.app`，可以直接双击运行，也可以复制到“应用程序”目录。

开发调试：

```bash
swift run --disable-sandbox
```

运行测试：

```bash
swift test --disable-sandbox
```

## 本地数据

首次启动会生成三条演示记录。正式数据默认存放在：

```text
~/Library/Application Support/AutumnJobs/job-data.json
```

可以在应用的“设置”页面导出 CSV 或创建完整 JSON 备份。
每次数据保存成功后还会在同一目录的 `job-data-backups` 子目录中创建自动快照，滚动保留最近 20 份，可在“设置”中恢复。

## 邮件识别

在邮件客户端中复制通知正文，打开侧栏的“邮件识别”，点击“读取剪贴板并识别”。应用会在本机提取通知类型、公司、岗位、时间、形式、地点、会议链接和联系人；核对识别结果后，可以写入已有投递流程，或用识别出的公司和岗位新建投递。邮件内容不会发送到服务器。
